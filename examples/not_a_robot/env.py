...
have independent seeded resets and page-owned status grading. Remote Neal mode
...
from .catalog import GAME_URL, VALID_ACTIONS
from .evaluator import GameState, classify_page
from .local_tasks import ASSETS, CATALOG, LOCAL_TASKS, LocalTaskServer, LocalTaskState
...
class NotARobotEnv(LiteBaseEnv, EnvServerResource):
    ...
    def __init__(
        ...
        *,
        ...
        if mode == "local":
            if local_task not in LOCAL_TASKS or target_level is not None:
            ...
            raise ValueError("local_task is only supported in local mode")
    ...
    def task_metadata(
        ...
        if mode == "local":
            ...
            if "reference" in LOCAL_TASKS[local_task]:
                provenance["reference"] = dict(LOCAL_TASKS[local_task]["reference"])
    ...
    def _runtime_metadata(self) -> LiteCUAMetadata:
        return self.task_metadata(
            self.target_level, self.mode, self._extra_tools, self.local_task, self.seed
        )
        ...
        try:
            if self.mode == "local":
                self._local_server = LocalTaskServer(
                    require_reference=LOCAL_TASKS[self.local_task]
                    .get("reference", {})
                    .get("fidelity")
                    == "captured_instance"
                )
                self._local_url = (
                    ...
                    + "/?"
                    + urlencode({"task": self.local_task, "seed": self.seed, "runner": 1})
                )
                self.recorder.emit(
                    "local_game_started",
                    url=self._local_url,
            ...
            if self.mode == "local":
                if (
                    ...
                    or self.state.task_id != self.local_task
                ...
                return LiteEnvObservation(
                    image=image, text=LOCAL_TASKS[self.local_task]["instruction"]
                )
    ...
    async def _read_local_state(self) -> None:
        ...
        try:
            ...
            for event in events[self._game_event_sequence :]:
                self.recorder.emit("game_event", **event)
        ...
        try:
            ...
            if self.mode == "local":
                ...
                if previous_status != self.state.status:
                    ...
                    self.recorder.image(raw, phase=phase, variant="before_terminal_transition")
                    raw = await self._page.screenshot(type="png", timeout=10_000)
            self.recorder.image(
                raw, phase=phase, variant="raw", viewport=list(self.display_resolution)
            )
            if self.mode != "local":
    ...
    async def _primitive(self, call: str, **args) -> None:
        ...
        event = self.recorder.emit(
            "input_started", call=call, args=args, tool_call_id=self._active_call_id
        )
        try:
            if call == "wait":
    ...
    async def step(self, actions: list[RuntimeEnvAction]) -> LiteEnvStepResult:
        ...
        for action, call_id in prepared:
            ...
            if self.mode == "local":
                ...
                if self.state.status in ("success", "failure"):
                    self.outcome, terminated = self.state.status, True
        ...
        if self.state.status == "access_blocked":
            ...
            self.outcome, terminated, truncated = self.state.status, True, False
        if (
            not terminated
            and not truncated
            and (
