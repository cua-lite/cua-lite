# Lite.CUAWorld — Collect With `qwen3_8_27b`

Teacher runbook for the `Qwen/Qwen3.8-27B` half of Lite.CUAWorld. Dataset-level
...
dataset runbook consumes:
    ...
    .data/rollout/lite.cuaworld/qwen3_8_27b/$COMMIT/<software>/train_annotated
...
Thinking stays OFF: run `scripts/configs/qwen3_8/default/lite.cuaworld.yaml`
...
for SW in <the 38 rolloutable softwares>; do
  uv run python scripts/rollout.py \
    ...
    --save-video false \
    --save-gif false \
    --config-path scripts/configs/qwen3_8/default/lite.cuaworld.yaml \
    --filter "lambda m: not m.others.get('exclude_reason')" \
    --log-root ".data/rollout/lite.cuaworld/qwen3_8_27b/$COMMIT/$SW"
done
```
...
for SW in <softwares>; do
  uv run python devs/data/lite.osworld/filter.py \
    --log-root ".data/rollout/lite.cuaworld/qwen3_8_27b/$COMMIT/$SW/train" \
    --out     ".data/rollout/lite.cuaworld/qwen3_8_27b/$COMMIT/$SW/train_annotated" \
