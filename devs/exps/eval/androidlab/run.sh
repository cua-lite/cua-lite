#!/usr/bin/env bash
# androidlab eval — single-model invocation.
#
# Routes --log-root to <repo-root>/.exps/eval/androidlab/<commit-ts>_<commit>/<run_id>/<slug>/.
...
#   CUDA_VISIBLE_DEVICES=<gpus> ./devs/exps/eval/androidlab/run.sh <model-id>
...
#     tens of minutes, log full of TimeoutExpired. Mitigations:
...
cd "$ROOT"
EVAL_ENV_ID="androidlab"
source "$ROOT/devs/exps/eval/utils/runtime_mode.sh"

SLUG="${MODEL//\//_}"
ENV_ROOT="$ROOT/.exps/eval/androidlab"
...
shopt -s nullglob
PIPELINE_PATHS=(
  devs/exps/eval/androidlab/run.sh
  lite/core
  lite/agents
  ...
  scripts/rollout.py
  scripts/configs/*/default/androidlab.yaml
)
shopt -u nullglob
...
case "$MODEL" in
  ...
  stepfun-ai/GELab-Zero-*)         CFG=scripts/configs/step_gui/default/androidlab.yaml ;;
