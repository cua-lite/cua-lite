#!/usr/bin/env bash
# screenspot_pro (grounding) eval — single-model invocation.
#
# Routes --log-root to <repo-root>/.exps/eval/screenspot_pro/<commit-ts>_<commit>/<run_id>/<slug>/.
...
#   CUDA_VISIBLE_DEVICES=<gpus> ./devs/exps/eval/screenspot_pro/run.sh <model-id>
...
#   CUDA_VISIBLE_DEVICES=0,1     ./devs/exps/eval/screenspot_pro/run.sh Qwen/Qwen3-VL-32B-Instruct
#   CUDA_VISIBLE_DEVICES=0,1     ./devs/exps/eval/screenspot_pro/run.sh Qwen/Qwen3.5-27B
#
...
# Pre-req: `uv run python lite/gym/envs/screenspot_pro/scripts/utils/download_tasks.py` to
# pre-warm the HuggingFace cache (one-time; otherwise the env auto-downloads on first use).
...
PIPELINE_PATHS=(
  ...
  lite/gym/base.py lite/gym/factory.py lite/gym/services.py lite/gym/wrappers.py
...
case "$MODEL" in
  ...
  Tongyi-MAI/MAI-UI-*)             CFG=scripts/configs/mai_ui/default/screenspot_pro.yaml ;;
